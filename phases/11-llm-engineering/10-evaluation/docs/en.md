...
- [mlabonne LLM Scientist -- Evaluation](https://github.com/mlabonne/llm-course#6-evaluation) -- the four-part framing this lesson uses (automated benchmarks, human eval, model-based eval, feedback signal) with Goodhart's-law caveat and pointers to lm-evaluation-harness and Lighteval
- [Hugging Face LLM Evaluation Guidebook](https://huggingface.co/spaces/OpenEvals/evaluation-guidebook) -- mlabonne's first recommended eval reference; practical advice on data contamination, metric selection, and reproducibility
